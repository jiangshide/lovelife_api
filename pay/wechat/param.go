...
// 获取微信支付正式环境Sign值
func getReleaseSign(apiKey string, signType string, bm util.BodyMap) (sign string) {
	var h hash.Hash
...
// 获取微信支付沙箱环境Sign值
func getSignBoxSign(mchId, apiKey string, bm util.BodyMap) (sign string, err error) {
	var (
...
func getSanBoxSignKey(mchId, nonceStr, sign string) (key string, err error) {
	reqs := make(util.BodyMap)
	reqs.Set("mch_id", mchId)
	reqs.Set("nonce_str", nonceStr)
	reqs.Set("sign", sign)

	keyResponse := new(getSignKeyResponse)
	_, errs := util.NewHttpClient().Type(util.TypeXML).Post(sandboxGetSignKey).SendString(generateXml(reqs)).EndStruct(keyResponse)
	if len(errs) > 0 {
...
// 生成请求XML的Body体
func generateXml(bm util.BodyMap) (reqXml string) {
