...
var COMMENT = "SELECT C.id,C.uid,C.content_id contentId,C.content,C.at,C.topic,C.urls,C.status,C.reason,C.create_time date,UP.nick,UP.icon,(SELECT name FROM zd_user_remarks WHERE uid=C.uid AND from_id=? ) remark,(SELECT count(1) FROM zd_comment_uid WHERE comment_id=C.id) num,(SELECT count(1) FROM zd_comment_praise WHERE comment_id=C.id AND coalesce(status,0) IN(1,2)) praiseNum,(SELECT status FROM zd_comment_praise WHERE comment_id=C.id AND uid=? ) praises,(SELECT reason FROM zd_report WHERE content_id=C.id AND source=4 AND uid=? ) reportr "//uid,uid,uid
var COMMENT_ORDER_BY="ORDER BY C.create_time DESC LIMIT ? OFFSET ? "

var COMMENT_ADD = "INSERT INTO zd_comment(uid,content_id,content,at,topic,urls,status,reason)VALUES(?,?,?,?,?,?,?,?)"//添加评论
