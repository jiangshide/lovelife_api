...
type Blog struct {
	...
	// At string `json:"at"`//@内容:content:id
	// Topic string `json:"topic"`//频道(话题)内容:content:id
...
var BLOG_CHANNEL_NEW=BlogSql+BLOG_COMMON+BLOG_LEFT+"WHERE B.status=2 AND (C.channel_nature_id = 2 OR (C.channel_nature_id = 1 AND C.uid=?)) AND C.id=? "+BLOG_SORT_DATE//针对频道最新:uid,uid,uid,uid,uid,uid,channel_id,limit,offset
var BLOG_CHANNEL_HOT=BlogSql+BLOG_COMMON+BLOG_LEFT+"WHERE B.status=2 AND (C.channel_nature_id = 2 OR (C.channel_nature_id = 1 AND C.uid=?)) AND C.id=? "+ORDER_BY_PRAISE//针对频道最热:uid,uid,uid,uid,uid,uid,channel_id,limit,offset
...
var BLOG_FOLLOW = BlogSql+BLOG_COMMON+BLOG_LEFT+"LEFT JOIN zd_user_follow UF ON UF.uid=B.uid AND UF.uid=UP.id WHERE (C.channel_nature_id = 2 OR (C.channel_nature_id = 1 AND C.uid=?)) AND (UF.from_id=? AND coalesce(UF.status,0) IN(1,2)) AND B.status=2 "+BLOG_SORT_DATE//关注:uid,uid,uid,uid,uid,uid,uid,limit,offset
var BLOG_FIND = BlogSql+BLOG_COMMON+BLOG_LEFT+"LEFT JOIN zd_blog_recommend BR ON BR.blog_id = B.id LEFT JOIN zd_user_follow UF ON (UF.uid=B.uid AND UF.uid=?) WHERE (C.channel_nature_id = 2 OR (C.channel_nature_id = 1 AND C.uid=?)) AND B.uid!=? AND B.status=2 AND coalesce(BR.status,0) = 0 AND coalesce(UF.status,0) = 0 "+BLOG_SORT_DATE//发现:uid,uid,uid,uid,uid,uid,uid,uid,limit,offset
...
var BLOG_UID = BlogSql+BLOG_COMMON+BLOG_USER_TOP+BLOG_LEFT+"WHERE B.uid=? AND B.status=2 AND C.channel_nature_id = 2 "+BLOG_SORT_DATE//通过用户ID查询:uid,uid,uid,uid,uid,uid,uid,limit,offset
var BLOG_UID_PRIVATE = BlogSql+BLOG_COMMON+BLOG_USER_TOP+BLOG_LEFT+"WHERE B.uid=? AND B.status=2 "+BLOG_SORT_DATE//通过用户ID查询:uid,uid,uid,uid,uid,uid,uid,limit,offset
...
var BLOG_NAME =BlogSql+BLOG_COMMON+BLOG_LEFT+"WHERE B.status=2 AND (instr(B.title,?) >0 OR instr(B.name,?) >0) "+BLOG_SORT_DATE//通过名称后缀查询:uid,uid,uid,uid,uid,title,name
var BLOG_NAME_FORMAT =BlogSql+BLOG_COMMON+BLOG_LEFT+"WHERE B.status=2 AND (instr(B.title,?) >0 OR instr(B.name,?) >0) AND B.format=? "+BLOG_SORT_DATE//通过名称后缀查询:uid,uid,uid,uid,uid,title,name,format


var BLOG_FORMAT=BlogSql+BLOG_COMMON+BLOG_LEFT+"WHERE C.channel_nature_id = 2 AND B.status=2 AND B.format=? "+BLOG_SORT_DATE//通过格式查询:uid,uid,uid,uid,uid,format
var BLOG_STATUS_PUBLIC =BlogSql+BLOG_COMMON+BLOG_LEFT+"WHERE B.status=? AND C.channel_nature_id = 2 "+BLOG_SORT_DATE//通过状态查询:uid,uid,uid,uid,uid,status
var BLOG_STATUS =BlogSql+BLOG_COMMON+BLOG_LEFT+"WHERE B.uid=? AND B.status=? "+BLOG_SORT_DATE//通过状态查询:uid,uid,uid,uid,uid,status

var BLOG_LEFT="FROM zd_blog B LEFT JOIN zd_channel C ON C.id = B.channel_id LEFT JOIN zd_user_profile UP ON B.uid=UP.id LEFT JOIN zd_user_friend ZUF ON ZUF.uid=B.uid "

var BLOG_COMMON = ",C.name channel,C.cover channelCover,(SELECT COUNT(1) FROM zd_blog_praise WHERE blog_id=B.id AND status IN(1,2) ) praiseNum,(SELECT COUNT(1) FROM zd_blog_view WHERE blog_id=B.id ) viewNum,(SELECT COUNT(1) FROM zd_blog_share WHERE blog_id=B.id AND status=1 ) shareNum,(SELECT COUNT(1) FROM zd_comment WHERE content_id=B.id)+(SELECT COUNT(1) FROM zd_comment_uid WHERE content_id=B.id) commentNum,(SELECT name FROM zd_user_remarks WHERE uid=B.uid AND from_id=? ) remark,(SELECT status FROM zd_blog_praise WHERE blog_id=B.id AND uid=? ) praises,(SELECT reason FROM zd_report WHERE content_id=B.id AND source=3 AND uid=? ) reportr,(SELECT status FROM zd_user_follow WHERE uid=B.uid AND from_id=? ) follows,(SELECT status FROM zd_blog_collection WHERE blog_id=B.id AND uid=? ) collections,(SELECT COUNT(1) from zd_blog_collection WHERE blog_id=B.id) collectionNum "

var BLOG_USER_TOP=",(SELECT status FROM zd_blog_top WHERE blog_id=B.id AND uid=? ) tops "

var BlogSql = "SELECT B.id,B.uid,B.channel_id channelId,B.position_id positionId,B.content,B.title,B.des,B.latitude,B.longitude,B.location_type locationType,B.country,B.city,B.position,B.address,B.city_code cityCode,B.ad_code adCode,B.time_zone timeCone,B.tag,B.status,B.reason,B.official,B.url,B.cover,B.dynamic_cover dynamicCover,B.name,B.sufix,B.format,B.duration,B.width,B.height,B.size,B.rotate,B.bitrate,B.sample_rate sampleRate,B.level,B.mode,B.wave,B.lrc_zh lrcZh,B.lrc_es lrcEs,B.source,B.create_time date,UP.icon,UP.nick,UP.sex,UP.age,UP.zodiac,UP.latitude,UP.longitude,UP.location_type locationType,UP.city ucity,UP.online,UP.online_id onlineId,UP.online_name onlineName "
...
//删除	
var BLOG_DELETE = "DELETE zd_blog,zd_file,zd_blog_praise,zd_blog_collection,zd_comment,zd_blog_recommend,zd_blog_top,zd_blog_view,zd_blog_share,zd_report FROM zd_blog LEFT JOIN zd_file ON zd_file.content_id=zd_blog.id LEFT JOIN zd_blog_praise ON zd_blog_praise.blog_id=zd_blog.id LEFT JOIN zd_comment ON zd_comment.content_id=zd_blog.id LEFT JOIN zd_blog_recommend ON zd_blog_recommend.blog_id=zd_blog.id LEFT JOIN zd_blog_top ON zd_blog_top.blog_id=zd_blog.id LEFT JOIN zd_blog_view ON zd_blog_view.blog_id=zd_blog.id LEFT JOIN zd_blog_share ON zd_blog_share.blog_id=zd_blog.id LEFT JOIN zd_report ON zd_report.content_id=zd_blog.id LEFT JOIN zd_blog_collection ON zd_blog.id=zd_blog_collection.blog_id WHERE zd_blog.id=?"
...
func Blogs(uid int64,sql string,ids interface{})(maps *[]orm.Params,id int64,err error){
	...
	for _,v := range (*maps){
		...
		v["urls"] = files
		comments,_,_ := SqlList(COMMENT_STATUS,[...]interface{}{uid,uid,uid,v["id"],10,0})
		// beego.Info("------comments:",comments," | err:",err," | ids:",ids)
		v["comments"] = comments
		res,_,_ := Sql("SELECT id,position,size,txt_color txtColor,bg_color bgColor,scroll FROM zd_blog_style WHERE blog_id=?",[...]interface{}{v["id"]})
		v["style"]=res
...
func AddBlogStyle(styleJson string,id int64){
	...
	if err := json.Unmarshal([]byte(styleJson),&blogStyle);err != nil{
		beego.Info("addBlogStyle~err:",err)
	}
	blogStyle.BlogId = id
	blogStyle.Add()
}
...
var BLOG_PRAISE = BlogSql+BLOG_COMMON+BLOG_LEFT+"LEFT JOIN zd_blog_praise BP ON BP.blog_id = B.id WHERE B.status = 2 AND BP.uid=? AND coalesce(BP.status,0) IN(1,2) "+BLOG_SORT_DATE//点赞:uid,uid,uid,uid,uid,uid,limit,offset
...
var BLOG_RECOMMEND =BlogSql+BLOG_COMMON+"FROM zd_blog_recommend BR LEFT JOIN zd_blog B ON BR.blog_id = B.id LEFT JOIN zd_user_profile UP ON (BR.uid=UP.id AND BR.blog_id=B.id) WHERE (C.channel_nature_id = 2 OR (C.channel_nature_id = 1 AND C.uid=?)) AND BR.uid=? AND BR.status in(1,2) "+BLOG_SORT_DATE//推荐:uid,uid,uid,uid,uid,uid,uid,limit,offset
var BLOG_RECOMMEND_FORMAT =BlogSql+BLOG_COMMON+"FROM zd_blog_recommend BR LEFT JOIN zd_blog B ON BR.blog_id = B.id LEFT JOIN zd_user_profile UP ON (BR.uid=UP.id AND BR.blog_id=B.id) WHERE (C.channel_nature_id = 2 OR (C.channel_nature_id = 1 AND C.uid=?)) AND BR.uid=? AND B.format=? AND BR.status in(1,2) "+BLOG_SORT_DATE//推荐:uid,uid,uid,uid,uid,uid,uid,format,limit,offset
...
type BlogView struct{
	...
	Nums int `json:"mums"`//浏览次数
}

var BLOG_BROWSE = BlogSql+BLOG_COMMON+BLOG_LEFT+"LEFT JOIN zd_blog_view BV ON BV.blog_id = B.id WHERE B.status = 2 AND BV.uid=? "+BLOG_SORT_DATE//浏览:uid,uid,uid,uid,uid,uid,limit,offset

func (this *BlogView) TableName() string {		
	return TableName("blog_view")
}

func (this *BlogView) Update() (int64, error) {
	return orm.NewOrm().Update(this)
}

func (this *BlogView) ReadOrCreates()(created bool, id int64, err error){
	blogView := BlogView{Uid:this.Uid,BlogId:this.BlogId,Num:this.Num,Nums:this.Nums}
	created,id,err = orm.NewOrm().ReadOrCreate(&blogView,"uid","blog_id")
	if !created{
		blogView.Nums=blogView.Nums+1
		id,err = blogView.Update()
	}
	return
}
