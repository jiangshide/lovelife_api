...
func (this *User) GetByName()  error {
	err := orm.NewOrm().QueryTable(this.TableName()).Filter("name", this.Name).One(this)
	if err != nil {
		return err
	}
	return  nil
...
type Profile struct {
	...
	Online int `json:"online"`//在线状态:1~在线,2~音乐,3~电影,4~游戏,5~学习,6~工作,-1~离线
	OnlineId int64 `json:"onlineId"`//在线状态资源ID
	OnlineName string `json:"onlineName"`//在线状态资源名称
	...
	ChannelId int64 `json:"channelId"`//默认公共频道id
	ImSign string `json:"imSign"`//Im鉴权
	PraiseNotice int `json:"praiseNotice"`//点赞通知:1~打开,0~关闭
	FollowNotice int `json:"followNotice"`//关注通知:1~打开,0~关闭
	BroweHomeNotice int `json:"broweHomeNotice"`//看主页通知:1~打开,0~关闭
	BroweChannelNotice int `json:"broweChannelNotice"`//看频道通知:1~打开,0~关闭
	BroweBlogNotice int `json:"broweBlogNotice"`//看动态通知:1~打开,0~关闭
	CreateChannelNotice int `json:"createChannelNotice"`//创建频道通知:1~打开,0~关闭
	CreateBlogNotice int `json:"createBlogNotice"`//发布动态通知:1~打开,0~关闭
...
var USER_ONLINE = USER_STATUS+"add P.online=? "+USER_ORDER_BY//在线状态:fromId,fromId,fromId,uid,status,online,limit,offset

var USER_STATUS=UserSql+USER_COMMON+USER_LEFT+"WHERE U.status=? "+USER_ORDER_BY//状态用户:fromId,fromId,fromId,uid,status,limit,offset

var USER_NAME = UserSql+USER_COMMON+USER_LEFT+"WHERE U.status=2 AND ( instr(U.name,?) >0 OR instr(P.nick,?) >0 ) "+USER_ORDER_BY//用户名称:fromId,fromId,fromId,uid,U.name,P.nick,limit,offset

var USER_ID=UserSql+USER_COMMON+USER_LEFT+"WHERE P.id=? "//通过id查询:fromId,fromId,fromId,uid,id

var USER_LEFT = "FROM zd_user_profile P LEFT JOIN zd_uc_user U ON U.id=P.id "
var USER_COMMON=",(SELECT COUNT(1) FROM zd_user_follow WHERE uid=P.id )fansNum,(SELECT COUNT(1) FROM zd_user_follow WHERE from_id=P.id ) followsNum,(SELECT COUNT(1) FROM zd_channel WHERE uid=P.id) channelsNum,(SELECT COUNT(1) FROM zd_blog WHERE uid=P.id ) blogsNum,(SELECT SUM(num) FROM zd_user_active WHERE uid=P.id ) activeNum,(SELECT name FROM zd_user_remarks WHERE uid=P.id AND from_id=? ) remark,(SELECT status FROM zd_user_friend WHERE uid=P.id AND from_id=? ) friends,(SELECT status FROM zd_user_follow WHERE uid=P.id AND from_id=?) follows,(SELECT reason FROM zd_report WHERE content_id=P.id AND uid=? ) reportr "

var UserSql = "SELECT P.id,P.union_id unionId,P.icon,P.level,P.score,P.vip,P.certification,P.nick,P.full_name fullName,P.phone,P.intro,P.birthday,P.sex,P.age,P.nation,P.zodiac,P.constellation,P.hobby,P.email,P.weixin,P.qq,P.weibo,P.alipay,P.latitude,P.longitude,P.location_type locationType,P.country,P.province,P.city,P.ad_code adCode,P.channel_id channelId,P.im_sign imSign,P.praise_notice praiseNotice,P.follow_notice followNotice,P.browe_home_notice broweHomeNotice,P.browe_channel_notice broweChannelNotice,P.browe_blog_notice broweBlogNotice,P.create_channel_notice createChannelNotice,P.create_blog_notice createBlogNotice,P.notice_type noticeType,P.create_time date,U.name,U.status,U.reason,P.online,P.online_id onlineId,P.online_name onlineName "
...
type Active struct{
	...
	Uid int64 `json:"uid"`
	Source int `json:"source"`//1.开始页面启动1活跃度(间隔需要6分钟),2.账号登录2活跃度(间隔需要30分钟),3.邀请朋友成功2活跃度,4.发布视频2个活跃度,5.发布音乐1活跃度,6.发布图片1活跃度,7.发布文章1活跃度,8.创建一个频道1活跃度
	ContentId int64 `json:"contentId"`//内容id
	Num int `json:"num"`//活跃值:1000活跃度=1梵币=1人民币
}
...
func  AddScore(uid int64,contentId int64,from int)(int,error){
	active := new(Active)
	active.Uid = uid
	...
	if active.Source == 1{
		...
		active.Num = 2
		diffTime = 2000
	}else if active.Source == 3{
		active.Num = 2
	}else if active.Source == 4{
		active.Num = 2
	}else if active.Source == 5{
		active.Num = 1
	}else if active.Source == 6{
		active.Num = 1
	}else if active.Source == 7{
		active.Num = 1
	}else if active.Source == 8{
		active.Num = 1
	}else{
		beego.Info("from err:",from)
		return -1,errors.New("from err")
	}
	var countRes CountRes
	err := SqlRawRow("SELECT * FROM (SELECT update_time FROM zd_user_active WHERE uid=? AND source=? ORDER BY update_time DESC LIMIT 0,1) S WHERE now()-S.update_time < ? ",[...]interface{}{active.Uid,active.Source,diffTime},countRes)
	if err != nil{	
		if _,err := active.Add();err != nil{//UPDATE zd_user_active SET uid=?,source=?,content_id=?,num=? WHERE update_time > ?
			beego.Info("err avtive add:",err)
			return -1,err
		}else{
			return active.UpdateScroe(active.Uid,active.Num)
		}
	}
	return -1,err
}

func (this *Active) UpdateScroe(id int64,score int)(int,error){
	profile := new(Profile)
	profile.Id = id
	if err := profile.Query();err != nil{
		beego.Info("score query err:",err)
		return 0,err
	}else{
		num := profile.Score+score
		if _,err := SqlRaw("UPDATE zd_user_profile SET score=? WHERE id=? ",[...]interface{}{num,id});err != nil{
			beego.Info("score update err:",err)
		}
		return num,err
	}
}
...
// var USER_FOLLOW=UserSql+USER_COMMON+"FROM zd_user_profile P LEFT JOIN zd_uc_user U ON U.id=P.id LEFT JOIN zd_user_follow F ON P.id=F.uid WHERE coalesce(F.from_id,0) != ? AND P.id != ? "+USER_ORDER_BY//可关注用户:uid,uid,uid,uid,uid,uid,limit,offset
var USER_FOLLOW=UserSql+USER_COMMON+"FROM zd_user_profile P LEFT JOIN zd_uc_user U ON U.id=P.id WHERE P.id != ? AND coalesce((SELECT status FROM zd_user_follow WHERE uid=P.id limit 0,1),0) = 0 "+USER_ORDER_BY//可关注用户:uid,uid,uid,uid,uid,limit,offset
...
func (this *User) GenValidateCode(width int) string {
...
func InitImId(id int64){
	sign,err := tencentyun.GenUserSig(sdkappid,key,strconv.FormatInt(id, 10),86400*180)
	if err == nil{
		SqlRaw("UPDATE zd_user_profile SET im_sign=?",[...]interface{}{sign})
	}
}
