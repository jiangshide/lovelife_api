...
var FILE ="SELECT id,content_id contentId,classify,cover,dynamic_Cover dynamicCover,url,name,sufix,format,duration,width,height,size,rotate,bitrate,sample_rate sampleRate,level,mode,wave,lrc_zh lrcZh,lrc_es lrcEs,source FROM zd_file WHERE content_id=? AND classify=? order by create_time DESC LIMIT ? OFFSET ?"
...
var BLOG_UPDATE_FILE_COVER="UPDATE zd_blog SET cover=?,dynamic_cover=?,url=?,name=?,sufix=?,format=?,duration=?,width=?,height=?,size=?,rotate=?,bitrate=?,sample_rate=?,level=?,mode=?,wave=?,lrc_zh=?,lrc_es=?,source=? WHERE id=?"
...
func AddFile(filesJson string,id,channelId int64){
	beego.Info("filesJson:",filesJson)
	if err := json.Unmarshal([]byte(filesJson),&files);err != nil{
		beego.Info("AddFile~err:",err)
	}
	for k,v := range files	{
		if k == 0{
			if len(v.LrcZh)==0 && v.Format == 1{
			...
			if cover == ""{
				...
				if _,err := SqlRaw(BLOG_UPDATE_FILE_COVER,[...]interface{}{cover,v.DynamicCover,v.Url,v.Name,v.Sufix,v.Format,v.Duration,v.Width,v.Height,v.Size,v.Rotate,v.Bitrate,v.SampleRate,v.Level,v.Mode,v.Wave,v.LrcZh,v.LrcEs,v.Source,id});err != nil{
					beego.Info("update~blogfile~err:",err)
				}
				SqlRaw("UPDATE zd_channel SET blog_cover=? WHERE id=?",[...]interface{}{cover,channelId})
			}
