...
type AppAds struct{
	...
	Type int `json:"type"`//类型:1~开屏,2~banner
	Platform int `json:"platform"`//平台:0~无限制, 1~android, 2~ios, 3~web
	Price int `json:"price"`//价格
	Cover string `json:"cover"`//封面
	Url string `json:"url"`//内容地址
	Status int `json:"status"`//状态:1:正常,2:下架,3:异常
	Count int `json:"count"`//展示次数
}

func (this *AppAds) TableName() string {
	return TableName("app_ads")
}

func (this *AppAds) Add() (int64, error) {
	return orm.NewOrm().Insert(this)
}
