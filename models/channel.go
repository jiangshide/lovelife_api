...
var CHANNEL_UID_PUBLIC=ChannelSql+CHANNEL_COMMON+CHANNEL_LEFT+"WHERE C.status=2 AND C.uid=? AND C.channel_nature_id=2 "+CHANNEL_ORDERBY_TIIME//通过用户ID查询:uid,uid,uid,uid,uid,limit,offset
...
var CHANNEL_UID_ALL_PUBLIC=ChannelSql+CHANNEL_COMMON+CHANNEL_LEFT+"WHERE C.uid=? AND C.channel_nature_id=2 "+CHANNEL_ORDERBY_TIIME//针对用户所有的频道:uid,uid,uid,uid,uid,limit,offset

var CHANNEL_SEARCH = ChannelSql+CHANNEL_COMMON+CHANNEL_LEFT+"WHERE C.status=2 AND C.channel_nature_id = 2 AND ( C.uid=? OR C.id=? OR instr(C.name,?) >0 ) "+CHANNEL_ORDERBY_TIIME//搜索:uid,uid,uid,uid,uid,id,name,limit,offset

var CHANNEL_FOLLOW=ChannelSql+CHANNEL_COMMON+CHANNEL_LEFT+" LEFT JOIN zd_user_follow UF ON UF.from_id=C.uid WHERE C.status=2 AND C.channel_nature_id = 2 AND coalesce(UF.status,0) IN(1,2) AND UF.uid=? "+CHANNEL_ORDERBY_TIIME//已关注的:uid,uid,uid,uid,uid,limit,offset
var CHANNEL_UNFOLLOW=ChannelSql+CHANNEL_COMMON+CHANNEL_LEFT+" LEFT JOIN zd_user_follow UF ON UF.from_id=C.uid WHERE C.status=2 AND C.channel_nature_id = 2 AND coalesce(UF.status,0) NOT IN(1,2) "+CHANNEL_ORDERBY_TIIME//可关注的:uid,uid,uid,uid,limit,offset
...
var CHANNEL_COMMON=",(SELECT COUNT(1) FROM zd_blog WHERE status =2 AND channel_id=C.id) blogNum,(SELECT name FROM zd_user_remarks WHERE uid=C.uid AND from_id=? ) remark,(SELECT status FROM zd_channel_top WHERE channel_id=C.id AND uid=? ) tops,(SELECT status FROM zd_user_follow WHERE from_id=C.uid AND uid=? ) follows,(SELECT reason FROM zd_report WHERE content_id=C.id AND uid=? ) reportr "
var ChannelSql="SELECT C.id,C.uid,C.channel_type_id typeId,C.channel_nature_id natureId,C.cover,C.blog_cover blogCover,C.name,C.des,C.status,C.official,C.reason,C.update_time date,P.icon,P.nick,P.latitude,P.longitude,P.location_type locationType,P.online,P.online_id onlineId,P.online_name onlineName "
...
func Channels(uid int64,sql string,ids interface{})(maps *[]orm.Params,id int64,err error){
	...
	if err != nil || id == 0{
		beego.Info("channels~err:",err," | id:",id)
		return
	}
	for _,v := range (*maps){
...
var ChannelDes = []string{"展现真实的自己!","我的地盘我做主!","我的世界我描述!","我的人生我规划!","我的生活我享受!","展现真实的自己!","我的地盘我做主!","我的世界我描述!","我的人生我规划!","我的生活我享受!"}

func InitChannel(id,uid int64,nick string)int64{
	if id <= 0{
		channel := new(Channel)
		channel.Uid = uid
		channel.ChannelNatureId = 2
		channel.Des = ChannelDes[rand.Intn(9)]
		channel.Status=2
		if nick == ""{
			profile := new(Profile)
			profile.Id = uid
			profile.Query()
			channel.Name = profile.Nick
			id,_ = channel.Add()
			SqlRaw("UPDATE zd_user_profile SET channel_id=? WHERE id=? ",[...]interface{}{id,uid})
			return id
		}else{
			channel.Name = nick+"的空间"
			id,_ = channel.Add()
			SqlRaw("UPDATE zd_user_profile SET channel_id=? WHERE id=? ",[...]interface{}{id,uid})
			return id
		}
...
func (this *Channel) Query() error {
	return orm.NewOrm().QueryTable(this.TableName()).Filter("id",this.Id).Filter("name",this.Name).One(this)
}
...
func InitChannelNature(){
	...
	if err != nil{
		beego.Info("InitChannelNature~err:",err)
	}
	...
	if err != nil{
		beego.Info("channelNatureComm~err:",err)	
	}
}
...
func InitUserChannelNature(uid int64,netInfo,device,ip string,action int){
	...
	list,_ := channelNature.List(20,0)
	beego.Info(list)	
	for _,ct := range list{
		beego.Info(ct.Id)
		...
		if err != nil{
			beego.Info("channelNatureUser~id:",id," | err:",err)
...
type ChannelType struct{
	...
	Status int `json:"status"`//状态:1~显示,0~隐藏
...
func InitChannelType(){
	...
	for k,v := range types{
		...
		if(err != nil){
			beego.Info("InitChannelType~err:",err)
		}
	}
}

var CHANNEL_TYPE_ID=ChannelSql+CHANNEL_COMMON+CHANNEL_LEFT+"WHERE C.status=2 AND C.channel_nature_id = 2 AND C.channel_type_id=? "+CHANNEL_ORDERBY_TIIME//频道所属类型:uid,uid,uid,uid,channel_type_id,limit,offset
var CHANNEL_NAME=ChannelSql+CHANNEL_COMMON+CHANNEL_LEFT+"WHERE C.status=2 AND C.channel_nature_id = 2 AND instr(C.name,?) >0 "+CHANNEL_ORDERBY_TIIME//频道所属类型:uid,uid,uid,uid,name,limit,offset
...
func (this *ChannelType) List(pageSize, offSet int) (list []*ChannelType, total int64) {
	query := orm.NewOrm().QueryTable(this.TableName())
	total, _ = query.Count()
	query.OrderBy("-id").Filter("status__in",1).Limit(pageSize, offSet).All(&list)
	return
}
...
var CHANNEL_TYPE = "select id,name,des,status,sequence from zd_channel_type WHERE status > 0  order by sequence"
...
var CHANNEL_OFFICIAL="SELECT * FROM ("+ChannelSql+CHANNEL_COMMON+CHANNEL_LEFT+") S WHERE S.blogNum > 0 AND S.status=2 AND S.natureId = 2 AND S.official IN(1,2) order by S.date DESC limit ? offset ? "//官方推荐:uid,uid,uid,uid,limit,offset

var CHANNEL_HOT=ChannelSql+CHANNEL_COMMON+CHANNEL_LEFT+"WHERE C.status=2 AND C.channel_nature_id = 2 having blogNum > 0 ORDER BY blogNum DESC limit ? offset ?"//频道所属类型:uid,uid,uid,uid,limit,offset

var CHANNEL_RECOMMEND_UID=ChannelSql+CHANNEL_COMMON+"FROM zd_channel_recommend CR LEFT JOIN zd_channel C ON C.id=CR.channel_id LEFT JOIN zd_user_profile P ON P.id=CR.uid WHERE C.status=2 AND C.channel_nature_id = 2 AND CR.uid=? AND CR.status IN(1,2) having blogNum > 0 "+CHANNEL_ORDERBY_TIIME//针对用户推荐～高~重点处理:uid,uid,uid,uid,uid,limit,offset
var CHANNEL_RECOMMEND_UNFOLLOW=ChannelSql+CHANNEL_COMMON+"FROM zd_channel_recommend CR LEFT JOIN zd_channel C ON C.id=CR.channel_id LEFT JOIN zd_user_profile P ON P.id=CR.uid WHERE C.status=2 AND C.channel_nature_id = 2 AND CR.status NOT IN(1,2) having blogNum > 0 "+CHANNEL_ORDERBY_TIIME//针对已推荐的推荐~中:uid,uid,uid,uid,limit,offset
var CHANNEL_RECOMMEND_ALL=ChannelSql+CHANNEL_COMMON+CHANNEL_LEFT+"WHERE C.status=2 AND C.channel_nature_id = 2 AND C.uid != ? having blogNum > 0 "+CHANNEL_ORDERBY_TIIME//推荐所有~低:uid,uid,uid,uid,uid,limit,offset
