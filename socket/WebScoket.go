...
const(
	...
	pongWait = 60*time.Second//Time allowed to read the next pong message from the peer.s
	pingPeriod = (pongWait*9)/10//Send pings to peer with this period. Must be less than pongWait.
	maxMessageSize=512//Maximum messsage size allowed from peer.
)

var maxConnId int64//最大的连接ID,每次连接抖加1处理 

/**
	客户端读写消息
*/
type wsMessage struct{
	messageType int//websocket.TextMessage 消息类型
	data []byte
}

var wsConnAll map[int64]*wsConnection//广播ws所有链接

var upgrader = websocket.Upgrader{
	ReadBufferSize:1024,
	WriteBufferSize:1024,
	CheckOrigin:func(r*http.Request)bool{//允许所有的CORS跨域请求，正式环境可以关闭
		return true
	},
}
